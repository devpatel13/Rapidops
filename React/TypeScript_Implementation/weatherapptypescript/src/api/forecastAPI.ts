import { ForecastList } from "../components/utils/type";

type FetchDataResponse = {
  status: Number;
  message?: String;
  tempForecastList?: ForecastList[];
};
const fetchData = async (
  ...
  try {
    ...
    const forecastResponse: any = await fetch(
    ...
    const tempForecastList: ForecastList[] = [];
    let key = 0;
    forecastResult.list.forEach((elem: any) => {
